...
上の図は論理的な呼び出し連鎖を表します。物理的なスタックフレームの数は
コンパイラの最適化で変わります。`-O2` では `ready_push()` / `ready_pop()` の
インライン化と末尾呼び出し最適化 (tail call) が起こり、`st_yield()` →
`switch_to_next()` → `st_ctx_swap()` が `jmp` の連鎖に簡約されます。このとき
スタックに積まれた戻りアドレスは worker が `st_yield` を呼んだ `call` の
1 つだけですが、それでも動作は同じです。`st_ctx_swap` が必要とするのは
「入口時点の `rsp` が直近の `call` の積んだ戻りアドレスを指す」ことだけ
であり、その戻り先から意味的に正しく続行できることに変わりはないからです。
これは呼出規約 (ABI) の上に構築された context switch が最適化に対して
堅牢であることを示しています。

本教材のビルド (`Makefile` の `CFLAGS`) は `-O0` に
`-fno-omit-frame-pointer` と `-fno-optimize-sibling-calls` を組み合わせ、
図のフレーム構造が実バイナリと一致するようにしています。gdb でスタックを
追う際もこの設定が前提です。
